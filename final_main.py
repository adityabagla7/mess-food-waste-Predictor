...
# Placeholder function to simulate API call (replace with actual Gemini API request)
def send_to_gemini_api(query, current_state):
    # Example: Replace this with your actual API endpoint and API key
    genai.configure(api_key="AIzaSyCq83cndnDrPI3c97OUQ_bmsnfv7xZS3G0")
    model = genai.GenerativeModel('gemini-1.5-pro')
    query = query
    query+= "This is the current state of the system after this line only if query is related to this answer or else just ignore this message and given response not related to the current state of the system." 
    query += current_state
    print(current_state)
    try:
